  ...
  */
   ...
   */
   ...
   for (auto& node : networkNodes){
    ...
    fflush(stdout);
    networkEnrich << node.name << "\t" << node.betweenessCentrality << "\t" << node.closenessCentrality << "\t" << node.degree << "\t" << node.numberOfDirectedEdges << "\t" << node.radiality << "\t" << node.topologicalCoefficient << "\t";

    for (auto& proteinGene : nodeInteractions[node.name]){
      networkEnrich << proteinGene.name << ",";
    }
    networkEnrich << "\t";
